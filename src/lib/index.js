(function (factory) {
  ...
  function getRelativePosition(point, canvas) {
    ...
    var { top, right, bottom, left } = canvasDom.getBoundingClientRect();
  ...
  function createEvent(event, chart) {
    ...
    if (event.touches) {
      ...
      var { x, y, clientX, clientY } = touch; // 小程序环境会有x,y，这里就直接返回
  ...
  class Base$1 {
    ...
    convertPoint(point) {
      var { x, y } = this._convertPoint(point);
      ...
      if (this.hasAdjust('stack') && coord.isPolar && coord.transposed) {
        if (invertPointX >= 0 && invertPointX <= 1) {
          ...
          tmp.forEach(function (obj) {
            if (
              isArray(yValue) ? obj[FIELD_ORIGIN_Y].toString() === yValue.toString() : obj[FIELD_ORIGIN_Y] === yValue
            ) {
              rst.push(obj);
            }
          });
  ...
  var LinearTick = function (cfg) {
    var { tickCount, tickInterval } = cfg || {};
    var { min, max } = cfg || {};
    min = isNaN(min) ? 0 : min;
    max = isNaN(max) ? 0 : max;
  ...
  function intervalIsVerify(_ref2) {
    var { interval, tickCount, max, min } = _ref2;
  ...
  class ScaleController {
    ...
    _adjustRange(type, cfg) {
      var { range, values } = cfg; // 如果是线性, 或者有自定义range都不处理
    ...
    createScale(field, data) {
      ...
      var { type, cfg } = this._getScaleCfg(field, data);
    ...
    _updateScale(scale) {
      var { field } = scale; // 因为每个field的数据都会不同

      var data = this.chart._getScaleData(field);

      var { cfg } = this._getScaleCfg(field, data);
    ...
    updateScales() {
      ...
      var { scales } = this; // 修改完列定义后，需要更新已经实例化的scale
    ...
    adjustStartZero(scale) {
      var { defs } = this;
      var { field, min, max } = scale; // 如果有定义，则不处理
  ...
  class Abastract {
    ...
    draw() {
      var { line, tickLine, label, grid } = this;
    ...
    drawLabels() {
      ...
      each(labels, function (labelShape) {
        ...
        var { x, y } = self.getSidePoint(start, labelOffset);
        labelShape.attr(
          mix(
            {
              x,
            ...
            self.getTextAlignInfo(start, labelOffset),
            labelShape.get('textStyle'),
          ),
        );
    ...
    drawGrid(grid) {
      ...
      each(gridPoints, function (subPoints, index) {
        ...
        if (gridCfg) {
          ...
          if (type === 'arc') {
            var { center, startAngle, endAngle } = self;
    ...
    getContainer(isTop) {
      var { frontContainer, backContainer } = this;
  ...
  class EventController {
    ...
    getEventType(points) {
      var { eventType, canvas, startTime, startPoints } = this;
  ...
  function _parseLineGradient(color, shape, context) {
    ...
    var { minX, minY, maxX, maxY } = shape.getBBox();
    var start;
    var end;
  ...
  function _parseRadialGradient(color, shape, context) {
    ...
    var { width, height, minX, minY } = shape.getBBox();
  ...
  class Rect extends Shape$2 {
    ...
    createPath(context) {
      var self = this;
      var attrs = self.get('attrs');
      var { x, y, width, height, radius } = attrs;
      context.beginPath();
  ...
  class ImageShape extends Shape$2 {
    ...
    drawImage(context, image) {
      var attrs = this.get('attrs');
      var { x, y, width, height, sx, sy, swidth, sheight } = attrs;
    ...
    calculateBox() {
      var attrs = this.get('attrs');
      var { x, y, width, height } = attrs; // 和rect一样
  ...
  class Line$1 extends Shape$2 {
    ...
    calculateBox() {
      var attrs = this.get('attrs');
      var { x1, y1, x2, y2, lineWidth } = attrs;
  ...
  class Polyline extends Shape$2 {
    ...
    calculateBox() {
      var attrs = this.get('attrs');
      var { points, smooth, lineWidth } = attrs;
  ...
  class Arc extends Shape$2 {
    ...
    createPath(context) {
      var attrs = this.get('attrs');
      var { x, y, r, startAngle, endAngle, anticlockwise } = attrs;
      context.beginPath();
  ...
  class Sector extends Shape$2 {
    ...
    createPath(context) {
      var attrs = this.get('attrs');
      var { x, y, startAngle, endAngle, r, r0, anticlockwise } = attrs;
      context.beginPath();
    ...
    calculateBox() {
      var attrs = this.get('attrs');
      var { x, y, r, r0, startAngle, endAngle, anticlockwise } = attrs;
  ...
  class Text extends Shape$2 {
    ...
    _getFontStyle() {
      var attrs = this._attrs.attrs;
      var { fontSize, fontFamily, fontWeight, fontStyle, fontVariant } = attrs;
      return ''
        .concat(fontStyle, ' ')
        .concat(fontVariant, ' ')
        .concat(fontWeight, ' ')
    ...
    calculateBox() {
      var self = this;
      var attrs = self._attrs.attrs;
      var { x, y, textAlign, textBaseline } = attrs;
    ...
    _getAxisCfg(coord, scale, verticalScale, dimType, defaultCfg) {
      ...
      each(ticks, function (tick, index) {
        ...
        if (labelCfg) {
          ...
          var { width, height } = axisLabel.getBBox();
    ...
    createAxis(coord, xScale, yScales) {
      ...
      if (chart._isAutoPadding()) {
        ...
        if (coord.isPolar) {
          ...
          if (circleAxis) {
            var { maxHeight, maxWidth, labelOffset } = circleAxis;
          ...
          if (axes.right && userPadding[1] === 'auto') {
            var { maxWidth: _maxWidth, labelOffset: _labelOffset } = axes.right;
            padding[1] += _maxWidth + _labelOffset;
          }

          if (axes.left && userPadding[3] === 'auto') {
            var { maxWidth: _maxWidth2, labelOffset: _labelOffset2 } = axes.left;
            padding[3] += _maxWidth2 + _labelOffset2;
          }

          if (axes.bottom && userPadding[2] === 'auto') {
            var { maxHeight: _maxHeight, labelOffset: _labelOffset3 } = axes.bottom;
      ...
      each(axes, function (axis) {
        var { type, grid, verticalScale, ticks, dimType, position, index } = axis;
        var appendCfg;
  ...
  function getClip(coord) {
    ...
    if (coord.isPolar) {
      var { circleRadius, center, startAngle, endAngle } = coord;
  ...
  var Dodge =
    ...
    (function (_Adjust) {
      ...
      _proto.getDodgeOffset = function getDodgeOffset(range, index, count) {
        ...
        var offset =
          (1 / 2) * (tickLength - count * width - (count - 1) * margin) +
  ...
  class Polar extends Base$1 {
    ...
    _invertPoint(point) {
      var self = this;
      var { center, transposed, x, y } = self;
      var xDim = transposed ? 'y' : 'x';
      var yDim = transposed ? 'x' : 'y';
      var m = [1, 0, 0, 1, 0, 0];
  ...
  class Circle$1 extends Abastract {
    ...
    drawLine(lineCfg) {
      var { center, radius, startAngle, endAngle } = this;
      var container = this.getContainer(lineCfg.top);
  ...
  class GuideBase {
    ...
    repaint() {
      this.remove();
      var { coord, container, canvas } = this;
  ...
  class Arc$1 extends GuideBase {
    ...
    render(coord, container) {
      ...
      var radius = Math.sqrt(
        (start.x - coordCenter.x) * (start.x - coordCenter.x) + (start.y - coordCenter.y) * (start.y - coordCenter.y),
      );
  ...
  class Html extends GuideBase {
    ...
    render(coord, container) {
      ...
      var { alignX, alignY, offsetX, offsetY } = self;
  ...
  class Text$1 extends GuideBase {
    ...
    render(coord, container) {
      ...
      var { content, style, offsetX, offsetY } = this;
  ...
  class Tag extends GuideBase {
    ...
    render(coord, container) {
      ...
      var { content, background, textStyle } = this;
      var shapes = [];
  ...
  class List {
    ...
    _renderBackground() {
      ...
      if (background) {
        ...
        var { minX, minY, width, height } = wrapper.getBBox();
    ...
    _addItem(item) {
      ...
      var { unCheckStyle, unCheckColor, nameStyle, valueStyle, wordSpace } = this;
      var { marker, value } = item;
      var startX = 0;
      ...
      if (name) {
        ...
        nameText = itemGroup.addShape('text', {
          ...
          attrs: mix(
            ...
            nameStyle,
            item.checked === false ? unCheckStyle : null,
          ),
        });
      }
    ...
    _getMaxItemWidth() {
      ...
      if (itemWidth === 'auto') {
        ...
        for (var i = 0; i < count; i++) {
          var { width: _width } = children[i].getBBox();
    ...
    _adjustHorizontal() {
      var { maxLength, itemsGroup } = this;
      var children = itemsGroup.get('children');
      var { itemGap, itemMarginBottom } = this;
    ...
    _adjustVertical() {
      var { maxLength, itemsGroup } = this;
      var { itemGap, itemMarginBottom, itemWidth } = this;
      var titleHeight = this._titleHeight;
      var children = itemsGroup.get('children');
      var colHeight = 0;
      var width;
  ...
  class TextBox {
    ...
    _init() {
      var { content, textStyle, background, className, visible, context } = this;
      var container = new Group({
        context,
        className,
        zIndex: 0,
    ...
    updateContent(text) {
      var { textShape, backgroundShape } = this;
      ...
      if (!isNil(text)) {
        ...
        var { x, y, width: tipWidth, height: tipHeight } = this._getBBox();
    ...
    updatePosition(x, y) {
      ...
      var { x: xMin, y: yMin } = this._getBBox();
  ...
  class Tooltip {
    ...
    constructor(cfg) {
      ...
      if (this.showXTip) {
        var { xTipBackground, xTipTextStyle } = this;
      ...
      if (this.showYTip) {
        var { yTipBackground, yTipTextStyle } = this;
    ...
    setXTipPosition(pos) {
      var { showXTip, canvas, plotRange, xTipBox, crosshairsShapeY } = this;
    ...
    setPosition(items) {
      var { container, plotRange, offsetX, offsetY, fixed, tooltipArrow } = this;
      ...
      var { minX, minY, width, height } = containerBBox;
      var { tl, tr } = plotRange;
      var posX = 0;
    ...
    _renderCrosshairs() {
      var { crosshairsType, crosshairsStyle, frontPlot, plotRange } = this;
      var { tl, br } = plotRange;
  ...
  class TooltipController {
    ...
    render() {
      ...
      var tooltipCfg = deepMix(
        ...
        defaultCfg,
        cfg,
      ); // 创建 tooltip 实例需要的配置，不应该修改 this.cfg，即用户传入的配置
    ...
    showTooltip(point) {
      ...
      var { showItemMarker, itemMarkerStyle, alwaysShow } = cfg;
      var marker;
    ...
    bindEvents() {
      var cfg = this._tooltipCfg;
      var { triggerOn, triggerOff, alwaysShow } = cfg;
  ...
  class GuideController {
    ...
    paint(coord) {
      var self = this;
      var { chart, guides, xScale, yScales } = self;
      var guideShapes = [];
  ...
  class LegendController {
      ...
      _defineProperty$3(this, 'handleEvent', function (ev) {
        ...
        function findItem(x, y) {
          ...
          each(legends, function (legendItems) {
            each(legendItems, function (legend) {
              var { itemsGroup, legendHitBoxes } = legend;
              var children = itemsGroup.get('children');
              ...
              if (children.length) {
                ...
                each(legendHitBoxes, function (box, index) {
                  if (
                    x >= box.x + legendPosX &&
                    x <= box.x + box.width + legendPosX &&
                    y >= box.y + legendPosY &&
                    y <= box.height + box.y + legendPosY
                  ) {
                    // inbox
                    result = {
        ...
        if (clicked && clicked.clickedLegend.clickable !== false) {
          ...
          if (clickedLegend.onClick) {
            ...
            var { filteredVals, field, selectedMode } = clickedLegend;
    ...
    addCustomLegend(field) {
      ...
      var legend = new List(
        deepMix({}, Global.legend[position], legendCfg, {
    ...
    _addCategoryLegend(scale, items, position, filteredVals) {
      var self = this;
      var { legendCfg, legends, container, chart } = self;
      var field = scale.field;
      legends[position] = legends[position] || [];
      var symbol = 'circle';
    ...
    _alignLegend(legend, pre, position) {
      ...
      if (position === 'left' || position === 'right') {
        ...
        y =
          position === 'top'
            ? appendPadding[0] + Math.abs(legend.container.getBBox().minY)
  ...
  class Timeline {
    ...
    update() {
      ...
      for (var i = 0; i < this.anims.length; i++) {
        ...
        var { startState, endState, interpolate, duration } = propertyAnim;
  ...
  var Animate = {
    ...
    registerAnimation(animationName, animationFun) {
      ...
      this.Action = _objectSpread$1(
        _objectSpread$1({}, this.Action),
        {},
        {
  ...
  function getScaledMatrix(shape, v, direct) {
    ...
    if (direct === 'x') {
      ...
      scaledMatrix = Matrix.transform(_matrix, [
        ['t', x, y],
        ['s', 1, 100],
        ['t', -x, -y],
      ]);
      ...
      scaledMatrix = Matrix.transform(_matrix2, [
        ['t', x, y],
        ['s', 100, 100],
        ['t', -x, -y],
      ]);
  ...
  function doAnimation(shape, endState, animateCfg, callback) {
    ...
    var { easing, delay, duration } = getAnimateParam(animateCfg, index, id);
  ...
  function _groupScaleIn(container, animateCfg, coord, zeroY, type) {
    var { start, end, width, height } = getCoordInfo(coord);
    var x;
    var y;
  ...
  function addAnimate(cache, shapes, canvas) {
    ...
    each(cache, function (deletedShape) {
      var { className, coord, _id, attrs, index, type } = deletedShape;
  ...
  function beforeCanvasDraw(chart) {
    ...
    var { frontPlot, backPlot } = chart.get('axisController');
  ...
  class Context {
    ...
    doZoom(leftScale, rightScale, zoom) {
      var { startRange: range, minScale } = this;
      var [start, end] = range;
    ...
    repaint(newValues) {
      var { chart } = this;
      var scale = this.getPinchScale();
      var { values: currentValues, ticks } = scale; // 如果新数组和当前显示的数组相同，则不更新
    ...
    updateFollowScale(pinchScale, pinchValues) {
      ...
      var { field: pinchField, type: pinchScaleType } = pinchScale;
      var { field: followField } = followScale; // 根据主轴的value值，找到所有从轴的value值
    ...
    updateTicks() {
      var { chart, values } = this;
      var scale = this.getPinchScale();
      var { values: currentValues, tickCount } = scale; // 根据当前数据的比例，和定义的tickCount计算应该需要多少个ticks
  ...
  class Base$2 {
    ...
    _bindEvents(chart) {
      var { startEvent, processEvent, endEvent, resetEvent } = this;
    ...
    _clearEvents() {
      var { chart, startEvent, processEvent, endEvent, resetEvent } = this;
      var canvas = chart.get('canvas'); // 统一绑定事件
